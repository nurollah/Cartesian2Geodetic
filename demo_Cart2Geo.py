...
#==========================================================================
import numpy as np
import GeometricGeodesy as gg
data = np.loadtxt('fi_la_h2.txt')
a = 6378137 # the semi-major axis of WGS84
b = 6356752.314 # the semi-minor axis of WGS84
e = np.sqrt((a**2-b**2)/a**2) # this is the eccentricity of the biaxial ellipsoid
p=gg.Geo2Cart(data,a,e)
G=gg.Cart2Geo(p,a,e)
dphi=np.max(np.deg2rad(G[:,0])-np.deg2rad(data[:,0]))
dlamda=np.max(np.deg2rad(G[:,1])-np.deg2rad(data[:,1]))
dh=np.max(G[:,2]-data[:,2])
print(dphi)
print(dlamda)
print(dh)